(ns forgerock.main
  "Kotodama WASM entrypoint for the Forgerock clean-room actor (L4) — Clojure port.
  ...
  facts (`forgerock.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "forgerock")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"forgerock.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "forgerock-compat" :tier tier :entities entities} 200])
...
(defn start! [] :forgerock-compat/ready)
